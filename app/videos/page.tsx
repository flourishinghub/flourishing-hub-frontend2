...
import { Play, Search, Eye, Clock, X } from 'lucide-react';
...
export default function VideosPage() {
  ...
  const handlePlay = async (video: Video) => {
    ...
    try { await apiCall(`/videos/${video.id}/view`, { method: 'POST' }); } catch {}
  };
  ...
  return (
    <DashboardLayout>
      <div>
        ...
        <p className="text-sm text-white/50 mt-1">Mentorship, leadership, and wellness content</p>
      </div>
          ...
          {(['ALL', 'MENTORSHIP', 'LEADERSHIP', 'WELLNESS'] as VideoCategory[]).map(cat => (
            <button key={cat} onClick={() => setCategory(cat)}
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold border transition-all ${
                category === cat ? 'bg-primary/20 text-primary border-primary/30' : 'bg-white/5 text-white/50 border-white/10 hover:bg-white/10'
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {filtered.map(video => {
            ...
            return (
              <motion.div key={video.id} whileHover={{ y: -3 }}
                className="glass-card rounded-2xl overflow-hidden cursor-pointer group" onClick={() => handlePlay(video)}>
                <div className="relative h-44 bg-gradient-to-br from-primary/20 to-accent/20 overflow-hidden">
                  {thumb && <img src={thumb} alt={video.title} className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-105" />}
        ...
        {playingVideo && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }}
              className="w-full max-w-3xl glass-card rounded-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
              <div className="flex items-center justify-between p-4 border-b border-white/10">
                <h3 className="text-sm font-semibold text-white line-clamp-1">{playingVideo.title}</h3>
                <button onClick={() => setPlayingVideo(null)} className="w-8 h-8 rounded-lg bg-white/10 hover:bg-white/20 flex items-center justify-center">
                  <X className="w-4 h-4 text-white/60" />
                </button>
              </div>
              <div className="aspect-video w-full bg-black">
                <iframe src={`https://www.youtube.com/embed/${getYoutubeEmbedId(playingVideo.youtubeUrl)}?autoplay=1`}
